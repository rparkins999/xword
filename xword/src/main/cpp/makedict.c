...
unsigned char bytes[3000000];
unsigned char * words [300000];
unsigned char * suffixes[300000];
int suffcounts[300000];
...
int main (int argc __attribute__ ((unused)), char argv[] __attribute__ ((unused)))
{
    ...
    fd = creat("xworddict.h", 0744);
    if (fd < 0)
    {
        fprintf(stderr, "creat(\"xworddict.h\", 0744) -> %s\n", strerror(errno));
        exit(1);
    }
    int err = write(fd, buff, snprintf(buff, sizeof(buff),
                    "#define MAXLEN %d\nunsigned int dict[] = {\n", maxlen));
    if (err < 0)
    {
        fprintf(stderr,
    ...
    for (i = 0; i < nw; ++i)
    {
        if (*(words[i]))
        {
            ...
            while (*p)
            {
                ...
                {
                    ++n32;
                    err = write(fd, buff, snprintf(buff, sizeof(buff), "0x%8.8x,\n", outword));
                    if (err < 0)
                    {
            ...
            if (suff)
            {
                err = write(fd, buff, snprintf(buff, sizeof(buff), "0x%8.8x, // %s(%s)\n", outword, words[i], suff));
            }
            else
            {
                err = write(fd, buff, snprintf(buff, sizeof(buff), "0x%8.8x, // %s\n", outword, words[i]));
            }
            if (err < 0)
            {
                fprintf(stderr, "write(\"xworddict.h\", %s) -> %s\n", buff, strerror(errno));
